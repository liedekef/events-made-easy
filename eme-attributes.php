...
function eme_attributes_form( $eme_array ) {
	...
	<?php if ( count( $attributes ) > 0 ) { ?> 
		<p><?php esc_html_e( 'Add attributes here', 'events-made-easy' ); ?></p>
		<table class="form-table eme_attributes">
		<thead>
			<tr>
				<th><strong><?php esc_html_e( 'Attribute Name', 'events-made-easy' ); ?></strong></th>
				<th><strong><?php esc_html_e( 'Value', 'events-made-easy' ); ?></strong></th>
			</tr>
		</thead>    
		<tbody id="eme_attr_body">
			...
			if ( is_array( $eme_data ) and count( $eme_data ) > 0 ) {
				foreach ( $eme_data as $name => $value ) {
					?>
					<tr id="eme_attr_<?php echo eme_esc_html( $count ); ?>">
					<td>
						<select name="eme_attr_<?php echo eme_esc_html( $count ); ?>_ref">
							<?php
					...
					<td>
			<textarea rows="2" cols="40" id="eme_attr_<?php echo eme_esc_html( $count ); ?>_id" name="eme_attr_<?php echo eme_esc_html( $count ); ?>_name"><?php echo eme_esc_html( $value ); ?></textarea>
					</td>
					</tr>
					<?php
					++$count;
				...
				?>
					...
					<td>
			<textarea rows="2" cols="40" id="eme_attr_<?php echo eme_esc_html( $count ); ?>_id" name="eme_attr_<?php echo eme_esc_html( $count ); ?>_name"></textarea>
					</td>
					</tr>
					<?php
			}
		...
		<tfoot>
			<tr>
				<td colspan="2"><a href="#" id="eme_attr_add_tag"><?php esc_html_e( 'Add new tag', 'events-made-easy' ); ?></a></td>
			</tr>
		</tfoot>
		</table>
		<?php
		...
		?>
		<p><?php esc_html_e( 'No attributes defined yet. If you want attributes, you first need to define/use some in the Settings page. See the section about custom attributes on the documention site for more info.', 'events-made-easy' ); ?></p>
		<?php
