...
/*
 * BounceRules — Bounce Mail Handler rule engine.
 * Based on PHPMailer-BMH (c) 2002-2009 Andy Prevost, GPL.
 *
 * Rules are matched in order; the first match sets rule_cat/rule_reason.
 * Data-driven rule table instead of a long if/elseif chain: each rule is
 * [pattern, category, reason, emailGroup?, target?], and a small "engine"
 * (findRule / applyRule) walks the list.
 *
 * Converted from free functions + globals to a class: the rule tables are
 * public properties, the engine is private methods - nothing here needs
 * `global` because everything is reached through $this.
 */
...
{
    /**
     * Default bounce_type/remove per rule category - a rule can override
     * either, otherwise these are the fallback.
     *
     * @var array<string, array{remove: int, bounce_type: string|false}>
     */
    public array $ruleCategories = [
    ...
    public array $dsnDiagRules = [
        ...
        ["/(?:alias|account|recipient|address|email|mailbox|user).*(?:n't|not)\s+exist/is", 'unknown', 'User does not exist'],
     ...
     * within Action: failed, after $dsnDiagRules found nothing.
     *
     * @var array
     */
    ...
    public function __construct()
    {
        ...
        ["/<(\S+@\S+\w)>.*\n?.*\n?.*account that you tried to reach does not\s+exist/i", 'unknown', 'Account does not exist (Gmail)', 1],
     ...
     */
    public function bodyRules(string $body, bool $debug_mode = false): array
    {
        $result = $this->emptyResult();

        $found = $this->findRule($this->bodyRules, ['body' => $body]);
        if ($found !== null) {
            $this->applyRule($found, $result);
            if ($result['email'] === '') {
                $result['email'] = $this->extractPrecedingEmail($body, $found['match'][0] ?? '');
            }
        }
     ...
     *               if the bounce type could NOT be detected, rule_reason stays ''
     */
    public function dsnRules(string $dsn_msg, string $dsn_report, bool $debug_mode = false): array
    {
        $result = $this->emptyResult();
        $result['email'] = self::parseDsnRecipient($dsn_report);
        ...
        if ($result['email'] === '') {
            $found = $this->findRule($this->dsnNoEmailRules, $subjects);
            if ($found !== null) {
                $this->applyRule($found, $result);
            }
        } else {
            ...
            switch ($action) {
                case 'failed':
                    $found = $this->findRule($this->dsnDiagRules, $subjects, $result, 'diag')
                        ?? $this->findRule($this->dsnMsgRules, $subjects, $result, 'dsn_msg');
                    if ($found !== null) {
                        $this->applyRule($found, $result);
                    }
                ...
                case 'delayed':
                    ...
                    $result['bounce_type'] = $this->ruleCategories['delayed']['bounce_type'];
                    $result['remove'] = $this->ruleCategories['delayed']['remove'];
    ...
    /**
     * Returns an empty result skeleton shared by bodyRules() and dsnRules().
     */
     ...
     */
    private function findRule(array $rules, array $subjects, array $context = [], string $defaultTarget = 'body'): ?array
    {
    ...
    /**
     * Applies a rule found by findRule() to $result: sets category, reason,
     * optionally the email address, and the bounce_type/remove pair (a rule
     * can override these; otherwise they come from $this->ruleCategories).
     */
    private function applyRule(array $found, array &$result): void
    {
        $rule  = $found['rule'];
        $match = $found['match'];
        ...
        $result['bounce_type'] = $rule['bounce_type'] ?? $this->ruleCategories[$rule['cat']]['bounce_type'];
        $result['remove']      = $rule['remove'] ?? $this->ruleCategories[$rule['cat']]['remove'];
    }
     ...
     */
    private function extractPrecedingEmail(string $body, string $matchedText): string
    {
        if ($matchedText === '') {
            return '';
        }
     ...
     * parseDsnRecipient() needs: pull a "mailbox@host" pair out of a string
     ...
     */
    private static function parseDsnRecipient(string $dsn_report): string
    {
        if (
            ...
            $email = trim($match[1], "<> \t\r\n\0\x0B");
            $parsed = self::parseAddress($email);
            if ($parsed['host'] !== '') {
