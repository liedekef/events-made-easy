...
function eme_check_captcha( $properties = [], $remove_captcha_if_ok = 0 ) {
    ...
    $selected_captcha = !empty($properties) && isset($properties['selected_captcha']) && isset($configured_captchas[$properties['selected_captcha']]) ? $properties['selected_captcha'] : '';
...
function eme_nl2br_save_html( $string ) {
    ...
    if ( $core !== '' ) {
        ...
        // to be sure: no br after a closing </li> or </td>
    ...
    if ( is_dir( $dir ) ) {
        ...
        if ( $handle ) {
            while ( ( $entry = readdir( $handle ) ) !== false ) {
                if ( $entry != '.' && $entry != '..' ) {
                    $info = explode( '-', $entry, 4 ); // max 4 parts (the rest is part of the name)
...
function eme_regenerate_extra_include_signatures() {
    $signatures = [];
    if ( file_exists( EME_EXTRA_INCLUDE_DIR ) && is_dir( EME_EXTRA_INCLUDE_DIR ) ) {
        foreach ( glob( EME_EXTRA_INCLUDE_DIR . '/eme_*.php' ) as $file ) {
            $signatures[ basename( $file ) ] = hash_file( 'sha256', $file );
        }
    }
    update_option( 'eme_extra_include_signatures', $signatures );
    $current_user = wp_get_current_user();
    $username = $current_user ? $current_user->user_login : 'unknown';
    error_log( "EME: Extra include signatures regenerated by user '{$username}'. Files: " . implode( ', ', array_keys( $signatures ) ) );
}
