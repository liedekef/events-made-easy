...
function eme_import_page() {
	...
	<div id="poststuff">
		...
		<p><?php esc_html_e( 'Import events from a CSV file. Required column: event_name. If providing categories, use the column category_names (pipe-separated, e.g. "Music||Concerts"). Optional: the columns eme_tasks and eme_todos contain tasks/todos as json (as generated by the events export), when present they fully replace the existing tasks/todos of the event; if not present they are left untouched.', 'events-made-easy' ); ?></p>
...
function eme_export_csv_events() {
	...
	$base_columns = [ 'event_name', 'event_start_date', 'event_end_date', 'price', 'currency', 'event_seats', 'event_external_ref', 'category_names', 'location_name', 'location_address1', 'location_city', 'location_latitude', 'location_longitude', 'location_external_ref', 'eme_tasks', 'eme_todos' ];
	...
	foreach ( $events as $event ) {
		...
		$row      = [
			$event['event_name'],
			substr( $event['event_start'], 0, 10 ),
			substr( $event['event_end'], 0, 10 ),
			$event['price'],
			$event['currency'],
			$event['event_seats'],
