...
function eme_get_active_member_by_personid_membershipid( $person_id, $membership_id ) {
    global $wpdb;
    $table         = EME_DB_PREFIX . EME_MEMBERS_TBNAME;
    $prepared_sql  = $wpdb->prepare( "SELECT * FROM $table WHERE person_id=%d AND membership_id=%d AND status IN (%d,%d) LIMIT 1", $person_id, $membership_id, EME_MEMBER_STATUS_ACTIVE, EME_MEMBER_STATUS_GRACE ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
    $member        = $wpdb->get_row( $prepared_sql, ARRAY_A ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
    $member        = eme_get_extra_member_data( $member );
    return $member;
}
...
function eme_get_member_by_wpid_membershipid( $wp_id, $membership_id, $status = EME_MEMBER_STATUS_ACTIVE.','.EME_MEMBER_STATUS_GRACE ) {
    ...
    if ( $status == 'all' ) {
        ...
        $status_arr = array_map( 'intval', explode( ',', $status ) );
        if ( count( $status_arr ) > 1 ) {
            $status_ph   = implode( ',', array_fill( 0, count( $status_arr ), '%d' ) );
            $cond_status = $wpdb->prepare( "AND members.status IN ($status_ph)", ...$status_arr ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
        } else {
            $cond_status = $wpdb->prepare( "AND members.status = %d", $status_arr[0] );
        }
    }
...
function eme_get_active_membershipids_by_wpid( $wp_id ) {
    ...
    $members_table = EME_DB_PREFIX . EME_MEMBERS_TBNAME;
    $people_table  = EME_DB_PREFIX . EME_PEOPLE_TBNAME;
    $prepared_sql  = $wpdb->prepare( "SELECT DISTINCT members.membership_id FROM $members_table AS members, $people_table AS people WHERE members.status IN (%d,%d) AND members.person_id=people.person_id AND people.wp_id=%d", EME_MEMBER_STATUS_ACTIVE, EME_MEMBER_STATUS_GRACE, $wp_id ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                    ...
                    if ( ! empty( $upload_failures ) ) {
                        ...
                        if ( isset( $_POST['familymember'] ) ) {
                            foreach ( wp_unslash( $_POST['familymember'] ) as $familymember ) {
                                ...
                                if ( $familymember_id ) {
                                    $familymember['member_id']         = $familymember_id;
...
function eme_admin_edit_memberform( $member, $membership_id, $limited = 0 ) {
    ...
    if ( $action == 'edit' ) {
        echo "<a href='" . esc_url( admin_url( 'admin.php?page=eme-people&eme_admin_action=edit_person&person_id=' . $member['person_id'] ) ) . "' title='" . esc_attr__( 'Click on this link to edit the corresponding person info', 'events-made-easy' ) . "'>" . esc_html__( 'Click on this link to edit the corresponding person info', 'events-made-easy' ) . '</a><br><br>';
    }
?>
...
?>
        ...
        <?php esc_html_e( 'If you want, select an existing person to become a member', 'events-made-easy' ); ?>
            ...
            <select id='chooseperson' name='chooseperson'
                ...
                class="eme_snapselect_autocompletememberperson nodynamicupdates">
            </select>
        </td></tr>
    <?php } else { ?>
<?php
...
function eme_get_sql_members_searchfields( $search_terms, $count = 0, $memberids_only = 0, $peopleids_only = 0, $emails_only = 0, $where_arr = [] ) {
    ...
    if ( ! current_user_can( get_option( 'eme_cap_list_members' ) ) ) {
        ...
        if ( empty( $member_ids_arr ) ) {
            ...
            $placeholders = implode(',', array_fill(0, count($ids_arr_int), '%d'));
            $where_arr[] = $wpdb->prepare( "(members.member_id IN ($placeholders))", ...$ids_arr_int ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
        }
    ...
    if ( ! empty( $search_terms['search_paymentid'] ) ) {
        $where_arr[] = $wpdb->prepare( "(payment_id = %d)", intval( $search_terms['search_paymentid'] ) );
    }
...
add_action( 'wp_ajax_eme_calc_memberprice', 'eme_calc_memberprice_ajax' );
add_action( 'wp_ajax_nopriv_eme_calc_memberprice', 'eme_calc_memberprice_ajax' );
...
add_action( 'wp_ajax_eme_dyndata_member', 'eme_dyndata_member_ajax' );
add_action( 'wp_ajax_nopriv_eme_dyndata_member', 'eme_dyndata_member_ajax' );
...
function eme_get_member_post_answers( $member, $include_dynamicdata = 1, $original_post_memberdata=[] ) {
    $answers = [];
    $membership_id = $member['membership_id'];
    ...
    if ( ! empty( $member['related_member_id'] ) ) {
        foreach ( $original_post_memberdata as $key => $value ) {
            if ( preg_match( '/^FIELD(\d+)$/', $key, $matches ) ) {
                $field_id = intval( $matches[1] );
                // keep the found field id, so we have it for the family member if it is present (and won't take it from the parent)
    ...
    foreach ( $_POST as $key => $value ) {
        if ( preg_match( '/^FIELD(\d+)$/', $key, $matches ) ) {
            $field_id = intval( $matches[1] );
            // the value was already stored for a family member, so don't do it again
...
function eme_store_member_answers( $member, $do_update = 1, $original_post_memberdata=[] ) {
    global $wpdb;
    ...
    $answer_ids_seen = [];
    $found_answers   = eme_get_member_post_answers( member: $member, original_post_memberdata: $original_post_memberdata );
...
function eme_get_person_ids_from_member_ids( $member_ids ) {
    global $wpdb;
    $members_table = EME_DB_PREFIX . EME_MEMBERS_TBNAME;
    $member_ids_arr = array_map( 'intval', explode( ',', $member_ids ) );
    $placeholders   = implode( ',', array_fill( 0, count( $member_ids_arr ), '%d' ) );
    $prepared_sql   = $wpdb->prepare( "SELECT person_id from $members_table WHERE member_id IN ($placeholders)", ...$member_ids_arr ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
    return $wpdb->get_col( $prepared_sql ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
}
...
function eme_member_recalculate_status( $member_id = 0 ) {
    ...
    if ( $member_id ) {
        $sql = $wpdb->prepare( "SELECT a.member_id, a.status, a.start_date, a.end_date, a.properties as memberprops, b.duration_period, b.properties FROM $members_table a LEFT JOIN $memberships_table b ON a.membership_id=b.membership_id WHERE b.status=1 AND a.member_id=%d AND a.status_automatic=1 AND a.paid=1 AND a.related_member_id=0", $member_id ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
    } else {
    ...
    foreach ( $memberships as $membership ) {
        ...
        if ( ! eme_is_empty_string( $membership['properties']['reminder_days'] ) ) {
            ...
            foreach ( $reminder_days as $reminder_day ) {
                ...
                $prepared_sql = $wpdb->prepare( "SELECT member_id from $table WHERE membership_id=%d AND related_member_id=0 AND status IN (%d,%d) AND DATEDIFF(end_date,%s)=%d", $membership_id, EME_MEMBER_STATUS_ACTIVE, EME_MEMBER_STATUS_GRACE, $today, $day ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
                $member_ids = $wpdb->get_col( $prepared_sql ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
                foreach ( $member_ids as $member_id ) {
...
function eme_renew_expired_member( $member, $pg = '', $pg_pid = '' ) {
    ...
    // we still set the renewal count +1, so we know this was a member in the past too
    $fields['renewal_count'] = $member['renewal_count'] + 1;
...
function eme_get_member_placeholder_handler_definitions() {
    ...
    $handlers = [
        ...
        '/#_TOTALDISCOUNT$/' => function( $result, $matches, $ctx ) {
            ...
            return eme_localized_price( $member['discount'], $ctx['membership']['properties']['currency'], $ctx['target'] );
        },
        ...
        '/#_ENDDATE\{(.+?)\}/' => function( $result, $matches, $ctx ) {
            ...
            if ( eme_is_expired_member( $member ) && $need_escape ) {
                $replacement = 'expired';
            } elseif ( eme_is_active_member( $member ) && ( $ctx['membership']['duration_period'] == 'forever' ) ) {
                if ( $need_escape ) {
                    $replacement = 'forever';
                } else {
        ...
        '/#_PAYMENTDATE\{(.+?)\}/' => function( $result, $matches, $ctx ) {
            return eme_localized_date( $ctx['member']['payment_date'], EME_TIMEZONE, $matches[1] );
        },
        '/#_PAYMENTDATE/' => function( $result, $matches, $ctx ) {
            return eme_localized_date( $ctx['member']['payment_date'], EME_TIMEZONE );
        },
        '/#_PAYMENTTIME\{(.+?)\}/' => function( $result, $matches, $ctx ) {
            return eme_localized_time( $ctx['member']['payment_date'], EME_TIMEZONE, $matches[1] );
        },
        '/#_PAYMENTTIME/' => function( $result, $matches, $ctx ) {
            return eme_localized_time( $ctx['member']['payment_date'], EME_TIMEZONE );
        },
        '/#_STATUS$/' => function( $result, $matches, $ctx ) {
            $eme_member_status_array = eme_member_status_array();
            return $eme_member_status_array[ $ctx['member']['status'] ];
        },
        '/#_IS_MEMBER_PENDING$/' => function( $result, $matches, $ctx ) {
            if ( $ctx['member']['status'] == EME_MEMBER_STATUS_PENDING ) {
                return 1;
            }
            return 0;
        },
        '/#_IS_MEMBER_ACTIVE$/' => function( $result, $matches, $ctx ) {
            if ( $ctx['member']['status'] == EME_MEMBER_STATUS_ACTIVE ) {
                return 1;
            }
            return 0;
        },
        '/#_IS_MEMBER_GRACE$/' => function( $result, $matches, $ctx ) {
            if ( $ctx['member']['status'] == EME_MEMBER_STATUS_GRACE ) {
                return 1;
            }
            return 0;
        },
        '/#_IS_MEMBER_EXPIRED$/' => function( $result, $matches, $ctx ) {
            if ( $ctx['member']['status'] == EME_MEMBER_STATUS_EXPIRED ) {
                return 1;
            }
            return 0;
        },
        '/#_FAMILYCOUNT/' => function( $result, $matches, $ctx ) {
            $related_member_ids = eme_get_family_member_ids( $ctx['member']['member_id'] );
            if ( ! empty( $related_member_ids ) ) {
        ...
        '/#_PDF_URL\{(\d+)\}/' => function( $result, $matches, $ctx ) {
            ...
            $generated_pdf = eme_generate_member_pdf( $member, $ctx['membership'], $template_id );
        ...
        '/#_PERSONAL_FILES/' => function( $result, $matches, $ctx ) {
            $res_files = [];
            $person_files = eme_get_uploaded_files( $ctx['member']['person_id'], 'people' );
    ...
    $handlers = [
        ...
        '/#_CONTACT/' => function( $result, $matches, $ctx ) {
            ...
            if ( $contact ) {
                ...
                return eme_replace_people_placeholders( $t_format, $person, $ctx['target'], $ctx['lang'], 0 );
            }
            return '';
        },
        '/#_FIELDNAME\{(.+?)\}/' => function( $result, $matches, $ctx ) {
        ...
        '/#_FIELD(VALUE)?\{(.+?)\}(\{.+?\})?/' => function( $result, $matches, $ctx ) {
            ...
            if ( ! empty( $formfield ) && $formfield['field_purpose'] == 'memberships' ) {
                ...
                foreach ( $ctx['answers'] as $answer ) {
                    if ( $answer['field_id'] == $field_id ) {
                        if ( $matches[1] == 'VALUE' ) {
                            $field_replace = eme_answer2readable( $answer['answer'], $formfield, 0, $sep, $ctx['target'] );
                        } else {
...
function eme_ajax_manage_memberships() {
    ...
    if ( isset( $_POST['do_action'] ) ) {
        ...
        case 'showMembershipStats':
            ...
            $ajaxResult['Result']      = 'OK';
            $ajaxResult['htmlmessage'] = eme_message_ok_div( $membershipstats );
            print wp_json_encode( $ajaxResult );
            break;
...
function eme_get_cf_membership_ids( $val, $field_id, $is_multi = 0 ) {
    ...
    $query_vars = [ $field_id, 'membership' ];
