...
';
    ...
    $eme_options = [
        ...
        'eme_events_page_title'                           => __( 'Events', 'events-made-easy' ),
        ...
        'eme_location_balloon_format'                      => "<strong>#_LOCATIONNAME</strong><br>#_ADDRESS - #_CITY<br><a href='#_LOCATIONPAGEURL'>Details</a>",
        'eme_location_map_icon'                           => '',
        'eme_osm_url'                                     => 'https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',
        'eme_osm_attribution'                             => 'Map data &copy; <a href="https://openstreetmap.org">OpenStreetMap</a>',
        ...
        'eme_recaptcha_api_key'                           => '',
        'eme_recaptcha_project_id'                        => '',
        'eme_recaptcha_score'                             => 0.5,
        'eme_cfcaptcha_for_forms'                         => 0,
        'eme_cfcaptcha_site_key'                          => '',
        'eme_cfcaptcha_secret_key'                        => '',
        'eme_friendlycaptcha_for_forms'                   => 0,
        'eme_friendlycaptcha_site_key'                    => '',
        'eme_friendlycaptcha_secret_key'                  => '',
        ...
        'eme_rsvp_paidmail_immediately'                   => 0,
        ...
        'eme_fs_contactperson_newevent_email_body'        => eme_br2nl( $contact_person_fs_newevent_email_body_localizable ),
        ...
        'eme_unsub_confirm_subject'                       => $eme_unsub_confirm_subject_localizable,
        'eme_unsub_confirm_body'                          => $eme_unsub_confirm_body_localizable,
        ...
        'eme_fs_payment_form_header_format'               => '',
        'eme_fs_payment_form_footer_format'               => '',
        ...
        'eme_imap_bounce_server'                          => '',
        'eme_imap_bounce_port'                            => 993,
        'eme_imap_bounce_username'                        => '',
        'eme_imap_bounce_password'                        => '',
...
function eme_update_options( $db_version ) {
    if ( $db_version ) {
        ...
        if ( $db_version < 366 ) {
            if ( get_option( 'eme_bancontactwero_button_img_url' ) == 'images/payment_gateways/payconiq/logo.png') {
                update_option( 'eme_bancontactwero_button_img_url', esc_url(EME_PLUGIN_URL) . 'images/payment_gateways/payconiq/logo.png' );
            }
        }
        if ( $db_version < 379 ) {
            $to_delete_options = [
        ...
        if ( $db_version < 419 ) {
            delete_option( 'eme_bancontactwero_merchant_id' );
            $rename_options = [
                'eme_location_baloon_format' => 'eme_location_balloon_format',
            ];
            foreach ( $rename_options as $old_option => $new_option ) {
                if ( get_option( $old_option ) ) {
                    update_option( $new_option, get_option( $old_option ) );
        ...
        if ( $db_version < 422 ) {
            $rename_options = [
                ...
                'eme_payconiq_cost'           => 'eme_bancontactwero_cost',
                'eme_payconiq_cost2'          => 'eme_bancontactwero_cost2',
                'eme_payconiq_button_label'   => 'eme_bancontactwero_button_label',
                'eme_payconiq_button_img_url' => 'eme_bancontactwero_button_img_url',
...
function eme_options_register() {
    ...
    $options = match( $tab ) {
        ...
        'people' => [ 'eme_full_name_format', 'eme_bd_email', 'eme_bd_email_members_only', 'eme_members_show_people_info' ],
        'memberships' => [ 'eme_page_access_denied', 'eme_membership_login_required_string', 'eme_redir_protected_pages_url', 'eme_membership_attendance_msg', 'eme_membership_unauth_attendance_msg' ],
...
function eme_admin_tabs( $current = 'homepage' ) {
    $tabs = [
        ...
        'memberships'   => __( 'Memberships', 'events-made-easy' ),
...
function eme_explain_slug_conflict( $conflict_found ) {
    ...
    ?>
        ...
        if ( $conflict_found === true ) {
            ...
            printf( wp_kses_post( __( 'The conflicting page can be edited <a href="%s" target="_blank" rel="noopener noreferrer">here</a>.', 'events-made-easy' ) ), esc_url( admin_url( "post.php?post=$conflict_found&action=edit" ) ) );
        }
        ?>
        </p></div>
    <?php
...
<table class="form-table">
            <?php
                ...
                eme_options_toggle( __( 'Use attributes?', 'events-made-easy' ), 'eme_attributes_enabled', __( 'Select yes to enable the attributes feature.', 'events-made-easy' ) );
                eme_options_toggle( __( 'Use membership feature?', 'events-made-easy' ), 'eme_members_enabled', __( 'Select yes to enable the members and membership features.', 'events-made-easy' ) );
                eme_options_toggle( __( 'Enable Maps?', 'events-made-easy' ), 'eme_map_is_active', __( 'Check this option to be able to show a map of the event or location using #_MAP or the available shortcodes.', 'events-made-easy' ) );
                eme_options_toggle( __( 'Always include JS in header?', 'events-made-easy' ), 'eme_load_js_in_header', __( 'Some themes are badly designed and can have issues showing the map or advancing in the calendar. If so, try activating this option which will cause the javascript to always be included in the header of every page (off by default).', 'events-made-easy' ) );
                eme_options_toggle( __( 'Use the client computer clock for the calendar', 'events-made-easy' ), 'eme_use_client_clock', __( 'Check this option if you want to use the clock of the client as base to calculate current day for the calendar.', 'events-made-easy' ) );
                eme_options_toggle( __( 'Use EME captcha for forms?', 'events-made-easy' ), 'eme_captcha_for_forms', __( 'Check this option if you want to use a simple image captcha on the booking/cancel/membership forms, to thwart spammers a bit. You can then either add #_CAPTCHA to your form layout yourself or it will automatically be added just above the submit button if not present.', 'events-made-easy' ) );
                eme_options_toggle( __( 'Use Google reCAPTCHA for forms?', 'events-made-easy' ), 'eme_recaptcha_for_forms', __( 'Check this option if you want to use Google reCAPTCHA (Cloud Enterprise) on the booking/cancel/membership forms, to thwart spammers a bit. Currently only supports reCAPTCHA v2 Tickbox. You can then either add #_CAPTCHA to your form layout yourself or it will automatically added just above the submit button if not present.', 'events-made-easy' ) );
                eme_options_input_text( __( 'Google reCAPTCHA site key', 'events-made-easy' ), 'eme_recaptcha_site_key', __( 'Go to https://console.cloud.google.com/security/recaptcha to get your reCAPTCHA key', 'events-made-easy' ) );
                eme_options_input_text( __( 'Google reCAPTCHA API key (Cloud project)', 'events-made-easy' ), 'eme_recaptcha_api_key', __( 'Go to https://console.cloud.google.com/apis/credentials to create a reCAPTCHA API key for your project', 'events-made-easy' ) );
                eme_options_input_text( __( 'Google reCAPTCHA Cloud project ID', 'events-made-easy' ), 'eme_recaptcha_project_id', __( 'Your Google cloud project ID', 'events-made-easy' ) );
                eme_options_input_text( __( 'Google reCAPTCHA minimum score (0.0–1.0, default 0.5)', 'events-made-easy' ), 'eme_recaptcha_score', __( 'Score threshold below which submissions are rejected', 'events-made-easy' ) );
                eme_options_toggle( __( 'Use hCaptcha for forms?', 'events-made-easy' ), 'eme_hcaptcha_for_forms', __( 'Check this option if you want to use hCaptcha on the booking/cancel/membership forms, to thwart spammers a bit. You can then either add #_CAPTCHA to your form layout yourself or it will automatically added just above the submit button if not present.', 'events-made-easy' ) );
                ...
                eme_options_toggle( __( 'Use Cloudflare Turnstile for forms?', 'events-made-easy' ), 'eme_cfcaptcha_for_forms', __( 'Check this option if you want to use Cloudflare Turnstile on the booking/cancel/membership forms, to thwart spammers a bit. You can then either add #_CAPTCHA to your form layout yourself or it will automatically added just above the submit button if not present.', 'events-made-easy' ) );
                ...
                eme_options_toggle( __( 'Use Friendly Captcha for forms?', 'events-made-easy' ), 'eme_friendlycaptcha_for_forms', __( 'Check this option if you want to use Friendly Captcha on the booking/cancel/membership forms, to thwart spammers a bit. You can then either add #_CAPTCHA to your form layout yourself or it will automatically added just above the submit button if not present.', 'events-made-easy' ) );
...
<table class="form-table">
<?php
            eme_options_select( __( 'Events page', 'events-made-easy' ), 'eme_events_page', eme_get_all_pages(), __( 'This option allows you to select which page to use as an events page.', 'events-made-easy' ) . '<br><strong>' . esc_html__( 'The content of this page (including shortcodes of any kind) will be ignored completely and dynamically replaced by events data.', 'events-made-easy' ) . '</strong>' );
            eme_options_toggle( __( 'Display events in events page?', 'events-made-easy' ), 'eme_display_events_in_events_page', __( 'This option allows to display a default list of events in the events page. It is not recommended to use this option, but use shortcodes on regular pages instead.', 'events-made-easy' ) );
            eme_options_toggle( __( 'Display calendar in events page?', 'events-made-easy' ), 'eme_display_calendar_in_events_page', __( 'This option allows to display an events calendar in the events page. It is not recommended to use this option, but use shortcodes on regular pages instead.', 'events-made-easy' ) );
...
<table class="form-table">
<?php
            eme_options_toggle( __( 'Hide past events?', 'events-made-easy' ), 'eme_cal_hide_past_events', __( 'Check this option if you want to hide past events in the calendar.', 'events-made-easy' ) );
            eme_options_toggle( __( 'Show single event?', 'events-made-easy' ), 'eme_cal_show_single', __( 'Check this option if you want to immediately show the single event and not a list of events if there is only one event on a specific day.', 'events-made-easy' ) );
?>
</table>
...
<?php
        ...
        case 'people':
?>
    <h2><?php esc_html_e( 'People Settings', 'events-made-easy' ); ?></h2>
    <table class='form-table'>
    <?php
        ...
        eme_options_toggle( __( 'Show custom people fields in members overview', 'events-made-easy' ), 'eme_members_show_people_info',
            __( 'To limit the number of fields shown/hidden in the overview of members, the custom fields of type "people" are not added by default. Check this option if you want those to be available in the members overview too.', 'events-made-easy' ) );
    ?>
    </table>
    <?php
        break;
...
<?php
            ...
            eme_options_input_text( __( 'RSS main description', 'events-made-easy' ), eme_get_field_name('eme_rss','main_description'), __( 'The main description of your RSS events feed.', 'events-made-easy' ), $rss_options['main_description'] );
            eme_options_input_text( __( 'RSS title format', 'events-made-easy' ), eme_get_field_name('eme_rss','title_format'), __( 'The format of the title of each item in the events RSS feed.', 'events-made-easy' ), $rss_options['title_format'] );
            eme_options_textarea( __( 'RSS description format', 'events-made-easy' ), eme_get_field_name('eme_rss','description_format'), __( 'The format of the description of each item in the events RSS feed. Follow the previous formatting instructions.', 'events-made-easy' ), $rss_options['description_format'] );
            eme_options_toggle( __( 'RSS Pubdate usage', 'events-made-easy' ), eme_get_field_name('eme_rss','show_pubdate'), __( 'Show the event creation/modification date as PubDate info in the in the events RSS feed.', 'events-made-easy' ), $rss_options['show_pubdate'] );
            eme_options_toggle( __( 'RSS Pubdate is start date', 'events-made-easy' ), eme_get_field_name('eme_rss','pubdate_startdate'), __( 'If you select this, the pubDate field in RSS will be the event start date, not the modification date.', 'events-made-easy' ), $rss_options['pubdate_startdate'] );
            eme_options_input_text( __( 'ICAL title format', 'events-made-easy' ), eme_get_field_name('eme_ical','title_format'), __( 'The format of the title of each item in the events ICAL feed.', 'events-made-easy' ), $ical_options['title_format'] );
            eme_options_input_text( __( 'ICAL description format', 'events-made-easy' ), eme_get_field_name('eme_ical','description_format'), __( 'The format of the description of each item in the events ICAL feed. Follow the previous formatting instructions.', 'events-made-easy' ), $ical_options['description_format'] );
            eme_options_input_text( __( 'ICAL location format', 'events-made-easy' ), eme_get_field_name('eme_ical','location_format'), __( 'The format of the location of each item in the events ICAL feed (if a location is defined for that event). Use any regular location placeholders.', 'events-made-easy' ), $ical_options['location_format'] );
?>
</table>
...
<?php
            ...
            eme_options_toggle( __( 'By default enable registrations for new events?', 'events-made-easy' ), 'eme_rsvp_reg_for_new_events', __( 'Check this option if you want to enable registrations by default for new events.', 'events-made-easy' ) );
            eme_options_toggle( __( 'By default require approval for registrations?', 'events-made-easy' ), 'eme_rsvp_require_approval', __( 'Check this option if you want by default that new registrations require approval.', 'events-made-easy' ) );
            eme_options_toggle( __( 'By default require user confirmation for registrations?', 'events-made-easy' ), 'eme_rsvp_require_user_confirmation', __( 'Check this option if you want by default that new registrations require confirmation of the person doing the booking.', 'events-made-easy' ) );
            ...
            eme_options_toggle( __( 'By default require WP membership to be able to register?', 'events-made-easy' ), 'eme_rsvp_registered_users_only', __( 'Check this option if you want by default that only logged-in users can book for an event.', 'events-made-easy' ) );
            eme_options_toggle( __( 'Check required fields upon submit?', 'events-made-easy' ), 'eme_rsvp_check_required_fields', __( 'Check this option if you want to check on the server-side if all required fields have been completed upon RSVP form submit. You might want to disable this if your form uses eme_if to show/hide certain form fields, otherwise your form might not get submitted if the hidden fields are marked as required.', 'events-made-easy' ) );
...
<?php
            ...
            eme_options_input_text( __( 'Email already registered text', 'events-made-easy' ), 'eme_rsvp_email_already_registered_string', __( 'The text shown if the email used to register is already used by another booking and the email is only allowed to register once.', 'events-made-easy' ) . '<br>' . sprintf( __( "For all placeholders you can use here, see <a target='_blank' rel='noopener noreferrer' href='%s'>the documentation</a>", 'events-made-easy' ), 'https://www.e-dynamics.be/wordpress/eme-docs/event-formatting/' ) );
            // translators: %s is the documentation URL
            ...
            eme_options_input_text( __( 'Attendees list format', 'events-made-easy' ), 'eme_attendees_list_format', __( 'The format for the attendees list when using the <code>#_ATTENDEES</code> placeholder.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all placeholders you can use here, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/attendees-list/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a>' );
            ...
            eme_options_input_text( __( 'Bookings list format', 'events-made-easy' ), 'eme_bookings_list_format', __( 'The format for the bookings list when using the <code>#_BOOKINGS</code> placeholder.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all placeholders you can use here, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/registration-form-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a>' . '<br>' . esc_html__( 'For more information about form fields, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/custom-attributes/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a>' );
            // translators: %s is the default format value
            ...
            eme_options_toggle( __( 'Show booking form again after booking?', 'events-made-easy' ), 'eme_rsvp_show_form_after_booking', __( "Uncheck this option if you don't want to show the booking form again after a successful booking.", 'events-made-easy' ) );
...
<table class='form-table'>
<?php
            eme_options_textarea( __( 'Booking form format', 'events-made-easy' ), 'eme_registration_form_format', __( 'The layout of the form for bookings. #_NAME, #_EMAIL and #_SEATS are obligated fields, if not present then the form will not be shown.', 'events-made-easy' ) . '<br>' . esc_html__( 'For more information about form fields, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/custom-attributes/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a>', 1 );
            eme_options_textarea( __( 'Cancel all bookings form format', 'events-made-easy' ), 'eme_cancel_form_format', __( 'The layout of the cancel form generated by [eme_cancel_all_bookings_form], used to cancel all bookings for one event. #_NAME and #_EMAIL are obligated fields, if not present then the form will not be shown.', 'events-made-easy' ) . '<br>' . esc_html__( 'For more information about form fields, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/cancel-registration-form-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a>', 1 );
            eme_options_textarea( __( 'Cancel one payment form format', 'events-made-easy' ), 'eme_cancel_payment_form_format', __( 'The layout of the cancel form generated when going to the #_CANCEL_URL (and related) placeholder, used to cancel all bookings related to 1 payment. #_SUBMIT, and #_CANCEL_PAYMENT_LINE (see the value of the next options below ) are the obligated fields, if not present then the form will not be shown.', 'events-made-easy' ) . '<br>' . esc_html__( 'For more information about form fields, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/cancel-one-payment-form/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a>', 1 );
...
<table class='form-table'>
<?php
            eme_options_toggle( __( 'By default require WP membership to be able to sign up for tasks?', 'events-made-easy' ), 'eme_task_registered_users_only', __( 'Check this option if you want by default that only logged-in users can sign up for tasks.', 'events-made-easy' ) );
            eme_options_toggle( __( 'By default require sign up approval for tasks?', 'events-made-easy' ), 'eme_task_requires_approval', __( 'Check this option if you want by default that signups for tasks need to be approved.', 'events-made-easy' ) );
            eme_options_toggle( __( 'By default allow only one sign up for tasks per event for a person?', 'events-made-easy' ), 'eme_task_only_one_signup_pp', __( 'Check this option if you want by default that a person can only signup for one task per event.', 'events-made-easy' ) );
            eme_options_toggle( __( 'Allow overlap for task signups?', 'events-made-easy' ), 'eme_task_allow_overlap', __( 'Check this option if you want to allow a person to sign up for tasks that overlap in time.', 'events-made-easy' ) );
            eme_options_textarea( __( 'Task signup form format (task entry part)', 'events-made-easy' ), 'eme_task_form_taskentry_format', __( 'The layout of one task people can select to signup for (repeated for each task).', 'events-made-easy' ) . '<br>' . __( 'This setting is used in the shortcode eme_tasks_signupform if the option "template_id" is not used for that shortcode.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/task-signup-form-placeholders/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a>' );
            eme_options_textarea( __( 'Task signup form format (personal info part)', 'events-made-easy' ), 'eme_task_form_format', __( 'The layout of the section of the task signup form where the info concerning lastname/firstname and email is collected.', 'events-made-easy' ) . '<br>' . __( 'This setting is used in the shortcode eme_tasks_signupform if the option "signupform_template_id" is not used for that shortcode.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/task-signup-form-placeholders/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a>' );
            eme_options_textarea( __( 'Task participants format', 'events-made-easy' ), 'eme_task_signup_format', __( 'The layout of one line in the list of signups for tasks (generated by #_TASKSIGNUPS or the eme_tasks_signups shortcode).', 'events-made-easy' ) . '<br>' . __( 'This setting is used in the shortcode eme_tasks_signups if the option "template_id" is not used for that shortcode.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/tasks-and-task-signup-placeholders/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a>' );
            eme_options_textarea( __( 'Signup recorded message', 'events-made-easy' ), 'eme_task_signup_recorded_ok_html', __( 'The text (html allowed) shown to the user when the task signup has been made successfully.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/tasks-and-task-signup-placeholders/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a>', 1 );
            #eme_options_textarea( __( 'Signup cancelled message', 'events-made-easy' ), 'eme_task_signup_cancelled_ok_html', __( 'The text (html allowed) shown to the user when the task signup has been successfully cancelled by the user.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/tasks-and-task-signup-placeholders/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a>', 1 );
...
<table class='form-table'>
<?php
            eme_options_toggle( __( 'Enable the RSVP email notifications?', 'events-made-easy' ), 'eme_rsvp_mail_notify_is_active', __( 'Check this option if you want to receive an email when someone books seats for your events.', 'events-made-easy' ) );
?>
</table>
<table id="rsvp_mail_notify-data" class='form-table'>
<?php
            eme_options_toggle( __( 'Enable pending RSVP emails?', 'events-made-easy' ), 'eme_rsvp_mail_notify_pending', __( 'Check this option if you want to send emails for pending bookings.', 'events-made-easy' ) );
            eme_options_toggle( __( 'Enable approved RSVP emails?', 'events-made-easy' ), 'eme_rsvp_mail_notify_approved', __( 'Check this option if you want to send emails for approved bookings.', 'events-made-easy' ) );
            eme_options_toggle( __( 'Enable paid RSVP emails?', 'events-made-easy' ), 'eme_rsvp_mail_notify_paid', __( 'Check this option if you want to send emails when a payment arrives via a payment gateway or when a booking is marked as paid.', 'events-made-easy' ) );
            eme_options_toggle( __( 'Send paid RSVP emails immediately?', 'events-made-easy' ), 'eme_rsvp_paidmail_immediately', __( 'Check this option if you want to send out the paid emails as soon as possible and not wait for the next queue run.', 'events-made-easy' ) );
?>
</table>
<table class='form-table'>
<?php
            eme_options_toggle( __( 'Send HTML emails', 'events-made-easy' ), 'eme_mail_send_html', __( 'Check this option if you want to use html in the emails being sent.', 'events-made-easy' ) );
            if ( eme_is_datamaster() ) {
                eme_options_toggle( __( 'Use mail queuing?', 'events-made-easy' ), 'eme_queue_mails', __( 'If activated, you can plan emails for sending at a later date and time.', 'events-made-easy' ) . '<br><b>' . esc_html__( 'It is recommended to activate this option.', 'events-made-easy' ) . '</b>' );
?>
...
<?php
            ...
            eme_options_toggle( __( 'Use SMTP authentication?', 'events-made-easy' ), 'eme_smtp_auth', __( 'SMTP authentication is often needed. If you use Gmail, make sure to set this parameter to Yes', 'events-made-easy' ) );
...
<?php
            ...
            eme_options_select(
                ...
                [
                    'ssl'    => 'SSL',
                    'tls'    => 'TLS',
                    'notls'  => __( 'None', 'events-made-easy' ),
                ],
...
?>
    <tr style='vertical-align:top'>
        ...
        <td>
        <button id="eme_run_bounce_handler" class="button button-primary" data-nonce="<?php echo esc_attr( wp_create_nonce( 'eme_admin' ) ); ?>"><?php esc_html_e( 'Process bounces now', 'events-made-easy' ); ?></button>
...
<?php
            $rest_url = rest_url( 'events-made-easy/v1/processbounces' );
            echo '<tr><th scope="row">' . esc_html__( 'REST API endpoint', 'events-made-easy' ) . '</th><td><code>' . esc_url( $rest_url ) . '</code>';
            echo '<br>' . esc_html__( 'This endpoint can be called externally (e.g. via WP-Cron) to trigger bounce processing. Requires authentication.', 'events-made-easy' ) . '</td></tr>';
?>
</table>
</div>
</details>
...
<table class='form-table'>
<?php
            eme_options_input_text( __( 'Booking Made Email Subject', 'events-made-easy' ), 'eme_respondent_email_subject', __( 'The subject of the email sent to the respondent when a booking is made (not pending) or approved.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/rsvp-mail-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ) );
            eme_options_textarea( __( 'Booking Made Email Body', 'events-made-easy' ), 'eme_respondent_email_body', __( 'The body of the email sent to the respondent when a booking is made (not pending) or approved.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/rsvp-mail-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ), $use_html_editor, $use_full );
            eme_options_input_text( __( 'Contact Person Booking Made Email Subject', 'events-made-easy' ), 'eme_contactperson_email_subject', __( 'The subject of the email that will be sent to the contact person when a booking is made.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/rsvp-mail-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ) );
...
<table class='form-table'>
<?php
            eme_options_input_text( __( 'Booking Awaiting User Confirmation Email Subject', 'events-made-easy' ), 'eme_registration_userpending_email_subject', __( 'The subject of the email that will be sent to the respondent if the booking requires user confirmation.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/rsvp-mail-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ) );
            eme_options_textarea( __( 'Booking Awaiting User Confirmation Email Body', 'events-made-easy' ), 'eme_registration_userpending_email_body', __( 'The body of the email that will be sent to the respondent if the booking requires user confirmation.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/rsvp-mail-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ), $use_html_editor, $use_full );
            eme_options_input_text( __( 'Booking Pending Email Subject', 'events-made-easy' ), 'eme_registration_pending_email_subject', __( 'The subject of the email that will be sent to the respondent if the booking requires approval.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/rsvp-mail-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ) );
            eme_options_textarea( __( 'Booking Pending Email Body', 'events-made-easy' ), 'eme_registration_pending_email_body', __( 'The body of the email that will be sent to the respondent if the booking requires approval.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/rsvp-mail-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ), $use_html_editor, $use_full );
            eme_options_input_text( __( 'Contact Person Pending Booking Email Subject', 'events-made-easy' ), 'eme_contactperson_pending_email_subject', __( 'The subject of the email that will be sent to the contact person if a booking requires approval.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/rsvp-mail-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ) );
...
<?php
            ...
            eme_options_input_text( __( 'Accepted Booking Reminder Email Subject', 'events-made-easy' ), 'eme_registration_reminder_email_subject', __( 'The subject of the email that will be sent to the respondent as a reminder of an approved booking.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/rsvp-mail-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ) );
            eme_options_textarea( __( 'Accepted Booking Reminder Email Body', 'events-made-easy' ), 'eme_registration_reminder_email_body', __( 'The body of the email that will be sent to the respondent as a reminder of an approved booking.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/rsvp-mail-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ), $use_html_editor, $use_full );
?>
</table>
</div>
</details>
...
<?php
            ...
            eme_options_input_text( __( 'Contact Person Cancelled Booking Email Subject', 'events-made-easy' ), 'eme_contactperson_cancelled_email_subject', __( 'The subject of the email that will be sent to the contact person when a respondent cancels all his bookings for an event.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/rsvp-mail-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ) );
            eme_options_textarea( __( 'Contact Person Cancelled Booking Email Body', 'events-made-easy' ), 'eme_contactperson_cancelled_email_body', __( 'The body of the email that will be sent to the contact person when a respondent cancels all his bookings for an event.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/rsvp-mail-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ), $use_html_editor, $use_full );
?>
</table>
</div>
</details>
...
<table class='form-table'>
<?php
            eme_options_input_text( __( 'Booking Paid Email Subject', 'events-made-easy' ), 'eme_registration_paid_email_subject', __( 'The subject of the email that will be sent to the respondent when a booking is marked as paid.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/rsvp-mail-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ) );
            eme_options_textarea( __( 'Booking Paid Email Body', 'events-made-easy' ), 'eme_registration_paid_email_body', __( 'The body of the email that will be sent to the respondent when a booking is marked as paid.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/rsvp-mail-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a>.<br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ), $use_html_editor, $use_full );
            eme_options_input_text( __( 'Contact Person Paid Email Subject', 'events-made-easy' ), 'eme_contactperson_paid_email_subject', __( 'The subject of the email that will be sent to the contact person when a booking is marked as paid (not via a payment gateway).', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/rsvp-mail-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ) );
            eme_options_textarea( __( 'Contact Person Paid Email Body', 'events-made-easy' ), 'eme_contactperson_paid_email_body', __( 'The body of the email that will be sent to the contact person when a booking is marked as paid (not via a payment gateway).', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/rsvp-mail-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ), $use_html_editor, $use_full );
?>
...
<table class='form-table'>
<?php
            eme_options_input_text( __( 'Contact Person New Event Email Subject', 'events-made-easy' ), 'eme_fs_contactperson_newevent_email_subject', __( 'The subject of the email that will be sent to the contact person when a new event is submitted.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/event-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ) );
            eme_options_textarea( __( 'Contact Person New Event Email Body', 'events-made-easy' ), 'eme_fs_contactperson_newevent_email_body', __( 'The body of the email that will be sent to the contact person when a new event is submitted.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/event-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ), $use_html_editor, $use_full );
            eme_options_input_text( __( 'Author Payment Notification Email Subject', 'events-made-easy' ), 'eme_fs_author_ipn_email_subject', __( 'The subject of the email that will be sent to the event author (the creator normally) when a payment notification is received via a payment gateway.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/event-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ) );
            eme_options_textarea( __( 'Author Payment Notification Email Body', 'events-made-easy' ), 'eme_fs_author_ipn_email_body', __( 'The body of the email that will be sent to the event author (the creator normally) when a payment notification is received via a payment gateway.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/event-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ), $use_html_editor, $use_full );
            eme_options_input_text( __( 'Contact Person Payment Notification Email Subject', 'events-made-easy' ), 'eme_fs_contactperson_ipn_email_subject', __( 'The subject of the email that will be sent to the contact person when a payment notification is received via a payment gateway.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/event-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ) );
...
<?php
            eme_options_input_text( __( 'Unsubscribe email subject', 'events-made-easy' ), 'eme_unsub_subject', __( 'The subject of the email that will be sent to the person asking to unsubscribe.', 'events-made-easy' ) . '<br>' . __( 'No placeholders can be used.', 'events-made-easy' ) );
            eme_options_textarea( __( 'Unsubscribe email body', 'events-made-easy' ), 'eme_unsub_body', __( 'The body of the email that will be sent to the person asking to unsubscribe.', 'events-made-easy' ) . '<br>' . __( 'Can contain all people placeholders and one additional required placeholder, namely #_UNSUB_CONFIRM_URL (which will be replaced with the unsubscribe confirmation url).', 'events-made-easy' ), $use_html_editor, $use_full );
            eme_options_input_text( __( 'Unsubscribe confirmation email subject', 'events-made-easy' ), 'eme_unsub_confirm_subject', __( 'The subject of the email that will be sent to the person as confirmation after they unsubscribe.', 'events-made-easy' ) . '<br>' . __( 'No placeholders can be used.', 'events-made-easy' ) );
            eme_options_textarea( __( 'Unsubscribe confirmation email body', 'events-made-easy' ), 'eme_unsub_confirm_body', __( 'The body of the email that will be sent to the person as confirmation after they unsubscribe.', 'events-made-easy' ), $use_html_editor, $use_full );
?>
</table>
</div>
</details>
...
<table class='form-table'>
<?php
            eme_options_input_text( __( 'Task Signup Pending Email Subject', 'events-made-easy' ), 'eme_task_signup_pending_email_subject', __( 'The subject of the email sent to the respondent when that person signs up for a task that requires approval.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/tasks-and-task-signup-placeholders/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ) );
            eme_options_textarea( __( 'Task Signup Pending Email Body', 'events-made-easy' ), 'eme_task_signup_pending_email_body', __( 'The body of the email sent to the respondent when that person signs up for a task that requires approval.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/tasks-and-task-signup-placeholders/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ), $use_html_editor, $use_full );
            eme_options_input_text( __( 'Contact Person Task Signup Pending Email Subject', 'events-made-easy' ), 'eme_cp_task_signup_pending_email_subject', __( 'The subject of the email that will be sent to the contact person when someone signs up for a task that requires approval.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/tasks-and-task-signup-placeholders/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ) );
            eme_options_textarea( __( 'Contact Person Task Signup Pending Email Body', 'events-made-easy' ), 'eme_cp_task_signup_pending_email_body', __( 'The body of the email that will be sent to the contact person when someone signs up for a task that requires approval.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/tasks-and-task-signup-placeholders/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ), $use_html_editor, $use_full );
?>
...
<table class='form-table'>
<?php
            eme_options_input_text( __( 'Task Signup Made Email Subject', 'events-made-easy' ), 'eme_task_signup_email_subject', __( 'The subject of the email sent to the respondent when that person signs up for a task.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/tasks-and-task-signup-placeholders/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ) );
            eme_options_textarea( __( 'Task Signup Made Email Body', 'events-made-easy' ), 'eme_task_signup_email_body', __( 'The body of the email sent to the respondent when that person signs up for a task.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/tasks-and-task-signup-placeholders/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ), $use_html_editor, $use_full );
            eme_options_input_text( __( 'Contact Person Task Signup Made Email Subject', 'events-made-easy' ), 'eme_cp_task_signup_email_subject', __( 'The subject of the email that will be sent to the contact person when someone signs up for a task.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/tasks-and-task-signup-placeholders/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ) );
            eme_options_textarea( __( 'Contact Person Task Signup Made Email Body', 'events-made-easy' ), 'eme_cp_task_signup_email_body', __( 'The body of the email that will be sent to the contact person when someone signs up for a task.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/tasks-and-task-signup-placeholders/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ), $use_html_editor, $use_full );
?>
...
<table class='form-table'>
<?php
            eme_options_input_text( __( 'Task Signup Cancelled Email Subject', 'events-made-easy' ), 'eme_task_signup_cancelled_email_subject', __( 'The subject of the email that will be sent to the respondent when he himself cancels a task signup.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/tasks-and-task-signup-placeholders/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ) );
            eme_options_textarea( __( 'Task Signup Cancelled Email Body', 'events-made-easy' ), 'eme_task_signup_cancelled_email_body', __( 'The body of the email that will be sent to the respondent when he himself cancels a task signup.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/tasks-and-task-signup-placeholders/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ), $use_html_editor, $use_full );
            eme_options_input_text( __( 'Contact Person Task Signup Cancelled Email Subject', 'events-made-easy' ), 'eme_cp_task_signup_cancelled_email_subject', __( 'The subject of the email that will be sent to the contact person when a respondent cancels a task signup.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/tasks-and-task-signup-placeholders/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ) );
            eme_options_textarea( __( 'Contact Person Task Signup Cancelled Email Body', 'events-made-easy' ), 'eme_cp_task_signup_cancelled_email_body', __( 'The body of the email that will be sent to the contact person when a respondent cancels a task signup.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/tasks-and-task-signup-placeholders/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If you leave this empty, this mail will not be sent.', 'events-made-easy' ), $use_html_editor, $use_full );
?>
</table>
</div>
</details>
...
<table class='form-table'>
<?php
            eme_options_input_text( __( 'Task Signup Deleted Email Subject', 'events-made-easy' ), 'eme_task_signup_trashed_email_subject', __( 'The subject of the email that will be sent to the respondent if the task signup is deleted by an admin.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/tasks-and-task-signup-placeholders/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a>' );
            eme_options_textarea( __( 'Task Signup Deleted Email Body', 'events-made-easy' ), 'eme_task_signup_trashed_email_body', __( 'The body of the email that will be sent to the respondent if the task signup is deleted by an admin.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/tasks-and-task-signup-placeholders/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a>', $use_html_editor, $use_full );
?>
</table>
</div>
</details>
...
<table class='form-table'>
<?php
            eme_options_input_text( __( 'Birthday Email Subject', 'events-made-easy' ), 'eme_bd_email_subject', __( 'The subject of the email that will be sent to people on their birthday (if active for them).', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/tasks-and-task-signup-placeholders/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a>' );
            eme_options_textarea( __( 'Birthday Email Body', 'events-made-easy' ), 'eme_bd_email_body', __( 'The body of the email that will be sent to  to people on their birthday (if active for them).', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/tasks-and-task-signup-placeholders/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a>', $use_html_editor, $use_full );
?>
</table>
</div>
</details>
...
<h2><?php esc_html_e( 'GDPR: General Data Protection Regulation options', 'events-made-easy' ); ?></h2>
            ...
            print esc_html__( 'For more info concerning GDPR, see', 'events-made-easy' ) . " <a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/eme_gdpr_approve/'>" . sprintf( esc_html__( 'the documentation about the shortcode %s', 'events-made-easy' ), esc_html( 'eme_gdpr_approve' ) ) . "</a>, <a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/eme_request_personal_info/'>" . sprintf( esc_html__( 'the documentation about the shortcode %s', 'events-made-easy' ), esc_html( 'eme_request_personal_info' ) ) . '</a> ' . esc_html__( 'and', 'events-made-easy' ) . " <a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/eme_change_personal_info/'>" . sprintf( esc_html__( 'the documentation about the shortcode %s', 'events-made-easy' ), esc_html( 'eme_change_personal_info' ) ) . '</a>'; ?>
<table class='form-table'>
<?php
    if ( get_option( 'eme_mail_send_html' ) == '1' ) {
        $use_html_editor = 1;
...
<?php
    ...
    eme_options_toggle( __( 'Redirect immediately to payment gateway?', 'events-made-easy' ), 'eme_pg_submit_immediately', __( 'By default, people get a list of payment gateways to choose from before being redirected to the payment gateway of choice. Select yes to immediately redirect to the payment gateway if there is only one payment gateway to chose from. This redirect takes also the setting  "Redirect wait period" into account. The text above the payment button of the relevant payment gateway will then also be shown.', 'events-made-easy' ) );
    eme_options_textarea( __( 'RSVP Payment form header format', 'events-made-easy' ), 'eme_payment_form_header_format', __( 'The format of the text shown above the payment buttons. If left empty, a default message will be shown.', 'events-made-easy' ) . ' ' . __( 'This option is only valid for event bookings, for memberships this can be defined for each membership individually.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/rsvp-mail-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a>', 1, 1 );
    ...
    eme_options_textarea( __( 'Multibooking payment form header format', 'events-made-easy' ), 'eme_multipayment_form_header_format', __( 'The format of the text shown above the payment buttons in the multibooking form. If left empty, a default message will be shown.', 'events-made-easy' ) . '<br>' . __( 'Although the same placeholders as for the regular payment form header format can be used, it is advised to only use multibooking related placeholders.', 'events-made-easy' ) , 1, 1 );
    eme_options_textarea( __( 'Multibooking payment form footer format', 'events-made-easy' ), 'eme_multipayment_form_footer_format', __( 'The format of the text shown below the payment buttons in the multibooking form. Default: empty.', 'events-made-easy' ) . '<br>' . __( 'Although the same placeholders as for the regular payment form header format can be used, it is advised to only use multibooking related placeholders.', 'events-made-easy' ), 1, 1 );
    eme_options_textarea( __( 'Frontend Event Submit Payment form header format', 'events-made-easy' ), 'eme_fs_payment_form_header_format', __( 'The format of the text shown above the payment buttons if the frontend submit form for events requires payment. If left empty, a default message will be shown.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/event-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a>', 1, 1 );
    eme_options_textarea( __( 'Frontend Event Submit Payment form footer format', 'events-made-easy' ), 'eme_fs_payment_form_footer_format', __( 'The format of the text shown below the payment buttons if the frontend submit form for events requires payment.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/event-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a>', 1, 1 );
    eme_options_textarea( __( 'Payment already done format', 'events-made-easy' ), 'eme_payment_booking_already_paid_format', __( 'The message shown instead of the payment form if payment has already been done.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/rsvp-mail-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a>', 1 );
    ...
    eme_options_textarea( __( 'Payment RSVP success return page format', 'events-made-easy' ), 'eme_payment_succes_format', __( 'The format of the return page when the payment is succesfull for bookings.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/event-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If left empty, a default message will be shown.', 'events-made-easy' ), 1 );
    eme_options_textarea( __( 'Payment RSVP failure return page format', 'events-made-easy' ), 'eme_payment_fail_format', __( 'The format of the return page when the payment failed or has been canceled for bookings.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/event-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If left empty, a default message will be shown.', 'events-made-easy' ), 1 );
    eme_options_textarea( __( 'Payment membership success return page format', 'events-made-easy' ), 'eme_payment_member_succes_format', __( 'The format of the return page when the payment is succesfull for membership signup.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/event-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If left empty, a default message will be shown.', 'events-made-easy' ), 1 );
    eme_options_textarea( __( 'Payment membership failure return page format', 'events-made-easy' ), 'eme_payment_member_fail_format', __( 'The format of the return page when the payment failed or has been canceled for membership signup.', 'events-made-easy' ) . '<br>' . esc_html__( 'For all possible placeholders, see ', 'events-made-easy' ) . "<a target='_blank' rel='noopener noreferrer' href='https://www.e-dynamics.be/wordpress/eme-docs/event-formatting/'>" . esc_html__( 'the documentation', 'events-made-easy' ) . '</a><br>' . __( 'If left empty, a default message will be shown.', 'events-made-easy' ), 1 );
    eme_options_toggle( __( 'Allow refunds?', 'events-made-easy' ), 'eme_payment_refund_ok', __( 'In case of a cancelled booking, the option to refund is presented (if the payment gateway supports it). If you want to disable this, select No.', 'events-made-easy' ) );
?>
</table>
</div>
</details>
...
<?php
    ...
    eme_options_toggle( __( 'Ignore Paypal tax setting?', 'events-made-easy' ), 'eme_legacypaypal_no_tax', __( 'Select yes to ignore the tax setting in your Paypal profile.', 'events-made-easy' ) );
    eme_options_input_text( __( 'Extra charge', 'events-made-easy' ), 'eme_' . $gateway . '_cost', __( 'Extra charge added to the price. Can either be an absolute number or a percentage. E.g. 2 or 5%', 'events-made-easy' ) );
    eme_options_input_text( __( 'Extra charge 2', 'events-made-easy' ), 'eme_' . $gateway . '_cost2', __( 'Second extra charge added to the price. Can either be an absolute number or a percentage. E.g. 2 or 5%', 'events-made-easy' ) );
    eme_options_input_text( __( 'Payment button label', 'events-made-easy' ), 'eme_' . $gateway . '_button_label', __( 'The text shown inside the payment button', 'events-made-easy' ) );
    eme_options_input_text( __( 'Payment button image', 'events-made-easy' ), 'eme_' . $gateway . '_button_img_url', __( 'The url to an image for the payment button that replaces the standard submit button with the label mentioned above.', 'events-made-easy' ) );
...
<?php
    ...
    eme_options_input_text( __( 'SumUp App ID', 'events-made-easy' ), 'eme_sumup_app_id', __( 'SumUp App ID', 'events-made-easy' ) . '<br><b>' . esc_html__( 'Contact SumUp to make sure your created application has the rights needed to create and process checkout payments.', 'events-made-easy' ) . '</b>') ;
    eme_options_input_text( __( 'SumUp App Secret', 'events-made-easy' ), 'eme_sumup_app_secret', __( 'SumUp App Secret', 'events-made-easy' ) . '<br><b>' . esc_html__( 'Contact SumUp to make sure your created application has the rights needed to create and process checkout payments.', 'events-made-easy' ) . '</b>' );
    eme_options_input_text( __( 'Extra charge', 'events-made-easy' ), 'eme_' . $gateway . '_cost', __( 'Extra charge added to the price. Can either be an absolute number or a percentage. E.g. 2 or 5%', 'events-made-easy' ) );
    eme_options_input_text( __( 'Extra charge 2', 'events-made-easy' ), 'eme_' . $gateway . '_cost2', __( 'Second extra charge added to the price. Can either be an absolute number or a percentage. E.g. 2 or 5%', 'events-made-easy' ) );
    eme_options_input_text( __( 'Payment button label', 'events-made-easy' ), 'eme_' . $gateway . '_button_label', __( 'The text shown inside the payment button', 'events-made-easy' ) );
    eme_options_input_text( __( 'Payment button image', 'events-made-easy' ), 'eme_' . $gateway . '_button_img_url', __( 'The url to an image for the payment button that replaces the standard submit button with the label mentioned above.', 'events-made-easy' ) );
...
<table class='form-table'>
<?php
    eme_options_toggle( __( 'Enable map scroll-wheel zooming?', 'events-made-easy' ), 'eme_map_zooming', __( '\'Yes\' enables map scroll-wheel zooming. \'No\' disables map scroll-wheel zooming. A browser refresh on a page showing a map will be necessary to see the effect of this change.', 'events-made-easy' ) );
    ...
    eme_options_input_text( __( 'Single location map zoom factor', 'events-made-easy' ), 'eme_indiv_zoom_factor', __( 'The zoom factor used when showing the map of a single location (max: 14).', 'events-made-easy' ) );
    ...
    eme_options_textarea( __( 'Default location balloon format', 'events-made-easy' ), 'eme_location_balloon_format', __( 'The format of the text appearing in the balloon describing the location in the map.', 'events-made-easy' ), 1 );
    eme_options_input_text( __( 'OpenStreetMap tile URL', 'events-made-easy' ), 'eme_osm_url', __( 'The URL of the OpenStreetMap tile server. The default is:: https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', 'events-made-easy' ) );
    eme_options_input_text( __( 'OpenStreetMap attribution', 'events-made-easy' ), 'eme_osm_attribution', __( 'The attribution text shown on the map. You can use HTML links.', 'events-made-easy' ) );
?>
</table>
<?php
    break;
...
printf( wp_kses_post( __( "For all information concerning frontend submit, see <a target='_blank' rel='noopener noreferrer' href='%s'>the documentation</a>", 'events-made-easy' ) ), esc_url( 'https://www.e-dynamics.be/wordpress/eme-docs/eme_add_event_form/' ) );
echo '<br><br>';
...
eme_options_select (__('Status for new event','events-made-easy'), eme_get_field_name('eme_fs','auto_publish'), eme_status_array(), __ ('The status for a newly submitted event.','events-made-easy'), $fs_options['auto_publish'] ?? false );
eme_options_input_text( __( 'Price', 'events-made-easy' ), eme_get_field_name('eme_fs','price'), __( 'The price to pay in order to submit a new event. Leave empty or 0 if no price is to be paid. This is not related to RSVP settings inside the submitted event itself! Configure the optional payment form header/footer sections in the EME Payment settings.', 'events-made-easy' ) . '<br>' . __( 'Use the point as decimal separator', 'events-made-easy' ), $fs_options['price'] ?? 0 );
...
//$indexed_users    += eme_get_indexed_users();
eme_options_select( __( 'Contact person', 'events-made-easy' ), eme_get_field_name('eme_fs','contact_person'), eme_get_indexed_users(), __( 'Select the contact person that will receive email notifications whenever an event gets submitted or paid for. The mail templates are to be configured in the regular mail templates section.', 'events-made-easy' ), $fs_options['contact_person'] ?? 0, __( 'No contact', 'events-made-easy' ) );
eme_options_select (__('Default category for new event','events-made-easy'), eme_get_field_name('eme_fs','default_cat'), $category_arr, __ ('The default category assigned to an event if nothing is selected in the form.','events-made-easy'), $fs_options['default_cat'] ?? 0 );
eme_options_toggle (__('Force location creation?','events-made-easy'), eme_get_field_name('eme_fs','force_location_creation'), __ ( 'Check this option if you want the location to be always created, even if the user does not have the needed capability set in EME to create locations.', 'events-made-easy' ), $fs_options['force_location_creation'] ?? false);
eme_options_toggle (__('Allow guest submit?','events-made-easy'), eme_get_field_name('eme_fs','guest_submit'), __ ( 'Check this option if you want guests also to be able to add new events.', 'events-made-easy' ), $fs_options['guest_submit'] ?? false );
...
eme_options_toggle (__('Use wysiwyg?','events-made-easy'), eme_get_field_name('eme_fs','use_wysiwyg'), __ ( 'Check this option if you want to use a frontend wysiwyg editor for the event notes.', 'events-made-easy' ), $fs_options['use_wysiwyg']);
eme_options_toggle (__('Allow image upload?','events-made-easy'), eme_get_field_name('eme_fs','allow_upload'), __ ( 'Check this option if you want to allow image upload in the frontend wysiwyg editor for the event notes.', 'events-made-easy' ), $fs_options['allow_upload'] ?? false);
...
<table class='form-table'>
<?php
    eme_options_toggle( __( 'Stay on edit page after save?', 'events-made-easy' ), 'eme_stay_on_edit_page', __( 'This allows you to stay on the edit page after saving events, locations, templates, formfields, people, groups or memberships.', 'events-made-easy' ) );
    eme_options_toggle( __( 'Remove leading zeros from minutes?', 'events-made-easy' ), 'eme_time_remove_leading_zeros', __( 'PHP date/time functions have no notation to show minutes without leading zeros. Checking this option will return e.g. 9 for 09 and empty for 00. This setting affects custom time placeholders and also the generic *DATE and *TIME placeholders', 'events-made-easy' ) );
    ...
    eme_options_toggle( __( 'Use external url for single events or locations?', 'events-made-easy' ), 'eme_use_external_url', __( 'If selected, clicking on the single event or location url for details will go to the defined external url for that event or location if present.', 'events-made-easy' ) );
