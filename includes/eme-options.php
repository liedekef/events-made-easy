...
function eme_options_delete() {
    global $wpdb;
    $wpdb->query( $wpdb->prepare( "DELETE FROM $wpdb->options WHERE option_name LIKE %s", 'eme_%' ) ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
}
...
//$indexed_users    += eme_get_indexed_users();
eme_options_select( __( 'Contact person', 'events-made-easy' ), eme_get_field_name('eme_fs','contact_person'), eme_get_indexed_users(), __( 'Select the contact person that will receive email notifications whenever an event gets submitted or paid for. The mail templates are to be configured in the regular mail templates section.', 'events-made-easy' ), $fs_options['contact_person'], __( 'No contact', 'events-made-easy' ) );
...
eme_options_toggle (__('Allow image upload?','events-made-easy'), eme_get_field_name('eme_fs','allow_upload'), __ ( 'Check this option if you want to allow image upload in the frontend wysiwyg editor for the event notes.', 'events-made-easy' ), $fs_options['allow_upload']);
