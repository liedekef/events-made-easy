            ...
            if ( eme_is_offline_pg( $pg ) ) {
                ...
                if ( function_exists( $func ) ) {
                    ...
                    if ( $eme_pg_submit_immediately && $pg != "braintree" ) { //braintree replaces our form, no need for submit/hidden
                        ...
                        // wp_add_inline_script is not working for ajax executed code
...
function eme_payment_form_webmoney( $item_name, $payment, $baseprice, $cur, $multi_booking = 0 ) {
    ...
    require_once EME_PLUGIN_DIR . 'payment_gateways/webmoney/webmoney.inc.php';
...
function eme_payment_form_opayo( $item_name, $payment, $baseprice, $cur, $multi_booking = 0 ) {
    ...
    require_once EME_PLUGIN_DIR . 'payment_gateways/opayo/eme-opayo-util.php';
...
function eme_payment_form_braintree( $item_name, $payment, $baseprice, $cur, $multi_booking = 0 ) {
    ...
    require_once EME_PLUGIN_DIR . 'payment_gateways/braintree/vendor/autoload.php';
...
function eme_payment_form_sumup( $item_name, $payment, $baseprice, $cur, $multi_booking = 0 ) {
    ...
    require_once EME_PLUGIN_DIR . 'payment_gateways/sumup/vendor/autoload.php';
    try {
...
function eme_payment_form_fdgg( $item_name, $payment, $baseprice, $cur, $multi_booking = 0 ) {
    ...
    require_once EME_PLUGIN_DIR . 'payment_gateways/fdgg/fdgg-util_sha2.php';
...
function eme_payment_form_legacypaypal( $item_name, $payment, $baseprice, $cur, $multi_booking = 0 ) {
    ...
    require_once EME_PLUGIN_DIR . 'payment_gateways/paypal_legacy/Paypal.php';
    $p = new Paypal();
...
function eme_payment_form_mercadopago( $item_name, $payment, $baseprice, $cur, $multi_booking = 0 ) {
    ...
    require_once EME_PLUGIN_DIR . 'payment_gateways/mercadopago/vendor/autoload.php';
...
function eme_complete_transaction_instamojo( $payment ) {
    ...
    $instamojo_key        = get_option( 'eme_instamojo_key' );
    $instamojo_auth_token = get_option( 'eme_instamojo_auth_token' );
    require_once EME_PLUGIN_DIR . 'payment_gateways/instamojo/vendor/autoload.php';
...
function eme_complete_transaction_stripe( $payment ) {
    $gateway = 'stripe';
    require_once EME_PLUGIN_DIR . 'payment_gateways/stripe/init.php';
...
function eme_complete_transaction_paypal($payment) {
    if ( ! class_exists( 'EME_PayPal_Client' ) ) {
        require_once EME_PLUGIN_DIR . 'payment_gateways/paypal/class_eme_paypal.php';
    }
    $client = new EME_PayPal_Client();
    if ( ! $client->is_configured() ) return 0;
    try {
        $paypal_orderid = $payment['pg_pid'];
        $response = $client->capture_order( $paypal_orderid );
        if ( $response['status'] === 'COMPLETED' ) {
            $capture_id = $response['purchase_units'][0]['payments']['captures'][0]['id'] ?? $paypal_orderid;
            $expected_price = eme_get_payment_price( $payment['id'] );
...
function eme_notification_paypal() {
    ...
    try {
        if ( ! $client->verify_webhook_signature( $payload, $headers ) ) {
    ...
    if ( $data && $data['event_type'] === 'CHECKOUT.ORDER.APPROVED' ) {
        $order_id = $data['resource']['id'];
        $payment = eme_get_payment_by_pg_pid( $order_id );
        if ( $payment ) {
            $res = eme_complete_transaction_paypal( $payment );
            if (!$res) {
...
function eme_complete_transaction_fondy( $payment ) {
    ...
    require_once EME_PLUGIN_DIR . 'payment_gateways/fondy/autoload.php';
...
function eme_notification_legacypaypal() {
    $gateway = 'legacypaypal';
    require_once EME_PLUGIN_DIR . 'payment_gateways/paypal_legacy/IPN.php';
    $ipn = new IPN();
    ...
    if ( $ipn->ipn_is_valid() ) {
        ...
        */
        ...
        // Verify receiver email matches configured business email
        $configured_email = get_option( 'eme_legacypaypal_business' );
        $receiver_email   = strtolower( $ipn->ipn['receiver_email'] ?? '' );
        if ( strtolower( $configured_email ) !== $receiver_email ) {
            $ipn->complete();
            return;
        }
...
function eme_paypal_webhook() {
    ...
    if ( strpos( $events_page_link, 'localhost' ) !== false ) {
        update_option( 'eme_paypal_webhook_error', __( 'Webhook not created: localhost not allowed.', 'events-made-easy' ) );
        return;
    }

    try {
        $result = $client->register_webhook( $notification_link );
        update_option( 'eme_paypal_webhook_id', $result['id'] );
        update_option( 'eme_paypal_webhook_error', '' );
    } catch ( Exception $e ) {
        update_option( 'eme_paypal_webhook_error', $e->getMessage() );
    }
}
...
function eme_charge_paypal() {
    ...
    $cancel_link = eme_payment_url($payment);
    try {
        $order = $client->create_order( $price, $cur, $description, $payment_id, $success_link, $cancel_link );
        eme_update_payment_pg_pid( $payment_id, $order['id'] );
...
function eme_charge_mercadopago() {
    ...
    $payment_id   = intval( $_POST['external_reference'] ?? 0 );
    $payment      = eme_get_payment( $payment_id );
    $success_link = eme_payment_return_url( $payment, 0 );
    $fail_link    = eme_payment_return_url( $payment, 1 );
...
function eme_charge_mollie() {
    ...
    if ( ! class_exists( 'Mollie\Api\MollieApiClient' ) ) {
        require_once EME_PLUGIN_DIR . 'payment_gateways/Mollie/vendor/autoload.php';
    }
...
function eme_notification_mollie( $mollie_payment_id = 0 ) {
    ...
    $payment    = eme_get_payment( $payment_id );
    if ( !$payment ) {
        return false; // payment doesn't exist, let's quit
    }
...
function eme_notification_worldpay() {
    ...
    $test_mode       = eme_sanitize_request( $_POST ['testMode'] ?? 0 );
...
function eme_mark_payment_paid( $payment_id, $is_ipn = 1, $pg = '', $pg_pid = '' ) {
    ...
    if ( $payment['target'] == 'member' ) {
        ...
        foreach ( $booking_ids as $booking_id ) {
            ...
            if ( !$mail_sent && $mailing_paid && ($total_price > 0 || ($total_price==0 && $booking_event_price>0) ) ) {
