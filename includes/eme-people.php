...
function eme_get_sql_people_searchfields( $search_terms, $count = 0, $ids_only = 0, $emails_only = 0, $where_arr=[] ) {
    ...
    if ( ! empty( $search_terms['search_groups'] ) && is_numeric( $search_terms['search_groups'] ) ) {
        ...
        if ( $tmp_group['type'] == "dynamic_people" ) {
            ...
            if (!empty($person_ids_arr)) {
                ...
                $where_arr[]  = $wpdb->prepare( "people.person_id IN ($placeholders)", ...$ids_arr_int ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
            }
    ...
    if ( ! empty( $formfields_searchable ) && ( $legacy_has_value || $legacy_has_fieldids ) ) {
        ...
        if ( count( $legacy_field_ids ) <= 1 ) {
            ...
            $where_arr[]  = $wpdb->prepare( "EXISTS (SELECT 1 FROM $answers_table WHERE related_id=people.person_id AND type='person' AND field_id IN ($placeholders) AND $cmp_sql)", ...$legacy_field_ids ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
            $cf_ids = [];
        }
    }
    ...
    if ( ! empty( $formfields_searchable ) && is_array( $cf_ids ) && ! empty( $cf_ids ) ) {
        ...
        foreach ( $cf_ids as $idx => $field_id ) {
            ...
            if ( $value === '' ) {
                $where_arr[] = $wpdb->prepare( "EXISTS (SELECT 1 FROM $answers_table WHERE related_id=people.person_id AND type='person' AND field_id=%d AND answer='')", $field_id ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
            } elseif ( ! empty( $cf_exact[ $idx ] ) ) {
                $where_arr[] = $wpdb->prepare( "EXISTS (SELECT 1 FROM $answers_table WHERE related_id=people.person_id AND type='person' AND field_id=%d AND answer=%s)", $field_id, $value ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
            } else {
                $where_arr[] = $wpdb->prepare( "EXISTS (SELECT 1 FROM $answers_table WHERE related_id=people.person_id AND type='person' AND field_id=%d AND answer LIKE %s)", $field_id, '%' . $wpdb->esc_like( $value ) . '%' ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
            }
        }
    }
...
function eme_people_birthday_emails() {
    ...
    if ( get_option( 'eme_bd_email_members_only' ) ) {
        ...
        $members_only  = $wpdb->prepare( " AND $members_table.status IN (%d,%d)", EME_MEMBER_STATUS_ACTIVE, EME_MEMBER_STATUS_GRACE ); // phpcs:ignore WordPress.DB.PreparedSQL.InterpolatedNotPrepared
    } else {
        $join         = '';
        $members_only = '';
    }
...
function eme_ajax_people_autocomplete( $no_wp_die = 0, $wp_membership_required = 0 ) {
    ...
    if ( $search_tables == 'people' || $search_tables == 'both' ) {
        ...
        if ( ! empty( $exclude_personids ) ) {
            ...
            $search .= $wpdb->prepare( " AND person_id NOT IN ($exc_ph)", ...$exc_ids ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
        }
        ...
        foreach ( $persons as $item ) {
            ...
            $record['birthdate'] = esc_html( $item['birthdate']);
            $return[]            = $record;
