...
zip -r $plugin.zip $plugin -x '*.git*' "$plugin/langs/*.po" "$plugin/langs/pot_gen*" "$plugin/langs/*.pot" "$plugin/langs/gettextize.sh*" "$plugin/dist*" "$plugin/changelog.txt" "$plugin/script*" "$plugin/payment_gateways/*/composer.lock" "$plugin/assets/readme_imgs*" "$plugin/*.md"
